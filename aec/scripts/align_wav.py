        ...
        for f in fs:
            if f.endswith("-mic.wav"):
                ...
                for i in range(len(mic_audio_segment) // len_of_samples):
                    mic_output = mic_audio_segment[i * len_of_samples: (i + 1) * len_of_samples]
                    lpb_output = lpb_audio_segment[i * len_of_samples: (i + 1) * len_of_samples]
                    mic_output.export(os.path.join(align_mic_output_path, real_name + f"-aligned-{i}-mic.wav"), format="wav")
                    lpb_output.export(os.path.join(align_lpb_output_path, real_name + f"-aligned-{i}-lpb.wav"), format="wav")
